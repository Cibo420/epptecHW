...
public class AlgorithmTestStreamReturn {
    ...
    public void testRemoveAllCandidates() {
        ...
        // Zavolání metody -> Act
        List<Person> result = Algorithm.returnFilteredPeople(candidates, Job.ENGINEER, 50, 1, 90, true, 160, 180);

        // Ověření, že všichni kandidáti byli odstraněni -> Assert
        assertEquals(0, result.size(), "Expected all candidates to be removed.");
    }
    @Test
    public void testKeepAllCandidates() {
        ...
        // Zavolání metody -> Act
        List<Person> result = Algorithm.streamRemovePeople(candidates, Job.TEACHER, 40, 0, 90, false, 150, 190);

        // Ověření, že žádný kandidát nebyl odstraněn -> Assert
        assertEquals(7, result.size(), "Expected no candidates to be removed.");
    }
    ...
    public void testRemoveTwoCandidates() {
        ...
        // Zavolání metody -> Act
        List<Person> result = Algorithm.streamRemovePeople(candidates, Job.TEACHER, 40, 0, 80, false, 150, 190);

        // Ověření, že jeden kandidát byl odstraněn -> Assert
        assertEquals(5, result.size(), "Expected one candidate to be removed.");
    }
}
